...
parameters {
    real<lower=0> lambda;                   #// sensitivity to differences in attraction scores
    vector[N_effects] mu;                   #// main effects of each parameter
...
model {
    ...
    for ( i in 1:N ) {                                                      #loop over all observations
        #//update attraction scores
        for ( j in 1:K ) {                                                  #loop over all in individuals
            if ( bout[i] > 1 ) {                                           
                AC[j] = (1-phi)*AC[j] + phi*y[i-1,j];
            } else {
                AC[j] = 0;                                                   #set attraction scores to being equal for first observation of an individual
            }
        }#//j

        if ( bout[i]==1 ) {
            #// calculate new individual's parameter values when a first bout is observed for each individual
        ...
        logPrA = lambda*AC[tech[i]] - log_sum_exp( lambda*AC );           // calculate individual learning probability on log(Pr) scale

        #//conformity aspect below
        if ( bout[i] > 1 ) {
            if (sum( s[i] ) > 0 ) {                                       //only use social info when social info is observed, otherwise fall back on individual learning
                ...
                for ( j in 1:K ) s_temp[j] = pow(s[i,j],fconf);            #exponents frequency cue to some frequency dependent parameter
                for ( j in 1:K ) lin_mod[j] = lin_mod[j] * s_temp[j];      #multiplies log linear component by frequency-dependent component

                PrS = lin_mod[tech[i]]/sum(lin_mod);                       #convert to raw probability
                
                target += ( log( (1-gamma)*exp(logPrA) + gamma*PrS ) );    # estimate probability on lob prob scale combining both, would make better in future
